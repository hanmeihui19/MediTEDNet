...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16, device='cuda'):
        super(ChannelAttention, self).__init__()
        self.ratio = ratio
        self.device = device
        self.fc = nn.Sequential(
            nn.Conv2d(in_planes, in_planes // self.ratio, 1, bias=False),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        avg_out = self.fc(F.adaptive_avg_pool2d(x, 1))  # (B, C, 1, 1)
        max_out = self.fc(F.adaptive_max_pool2d(x, 1))  # (B, C, 1, 1)
        out = avg_out + max_out  # (B, C, 1, 1)
        out = self.sigmoid(out)  # (B, C, 1, 1)
        out = out.permute(0, 2, 3, 1)  # (B, 1, 1, C)
        return out.expand(B, H, W, C)  # (B, H, W, C)


class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7, device='cuda'):
...
class Bi_Conv_VSS(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.ln = norm_layer(hidden_dim//2)
        self.SS2D = SS2D(d_model=hidden_dim//2, dropout=attn_drop_rate, d_state=d_state, **kwargs)
    ...
    def forward(self, input: torch.Tensor):
        input_conv, input_ssm = input.chunk(2,dim=-1)

        output_ssm = self.drop_path(self.SS2D(self.ln(input_ssm)))

        output_conv = input_conv.permute(0,3,1,2).contiguous()
        output_conv = self.Conv_branch(output_conv)
        output_conv = output_conv.permute(0,2,3,1).contiguous()
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=3, depths=[2, 2, 12, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[128,256,512,1024], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True, mid_fea=1000, fea_num=512,
